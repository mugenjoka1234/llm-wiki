...
- `team_ops.py search-candidates --query "<terms>" [--division D] [--source starter|index|references|all]` → ranked JSON `[{name, source, division, description, path, score}]`. Scoring: keyword/tag overlap — simple, documented, deterministic; ties broken by source priority (starter > references > vendored catalog — the user's own curated references outrank the generic catalog) then name; zero matches returns an empty list with `"suggestions": [<division names>]` so the skill can widen the search rather than fabricate. Reads frontmatter from: the vendored catalog (division = top-level directory), the starter roster, and factory-home `references/**`.
...
- **Layered resolution** in `team_ops.py` — a real signature change, stated as such: `resolve-team` (and solo lookup) gain an optional `--wiki-root <path>`; when given, member files resolve `<wiki-root>/personas/<slug>.md` first, then `<factory-home>/agents/<slug>.md` — full-file supersession by slug, no merging. Each resolved member (and `assemble-context`'s manifest) carries a `layer: project|factory` field, derived from the resolved path's prefix (under `<wiki-root>/personas/` → project). The `/team` skill already resolves the wiki in Preflight and passes it through — and its member-validation step MUST pass `--project <current>` for any member resolved from the project layer (otherwise the copy's own project mention denylist-fails and the member is wrongly benched — the run-time half of the layering rule). Solo invocation routes through the same layered resolver (`--wiki-root` on solo lookup), so "Wren, what do you think" inside a project gets that project's copy. (Verified: a `personas/` subdirectory at wiki root is already invisible to the wiki lint's scans — no lint change needed.)
- **Layer-aware improvement (supersedes "copies are the user's problem"):** the user is never expected to recall which layer their feedback targets — the process tracks it.
  - Project copies record provenance: `base-slug:`, `forked:` date, and `base-hash:` (base file sha256 at fork/ack time).
  - New `team_ops.py list-copies <slug>`: scans registered wikis' `personas/` dirs for copies with matching `base-slug` — a deterministic routing lookup (distinct from the cut drift-detection-inside-improve design).
  - Jot lines gain optional `wiki:` provenance (written by session-close, which knows the resolved wiki) so feedback arrives pre-routed.
  - `/improve` proposals carry an explicit routing question per edit — base / the specific project copy / both — defaulting from the jot's wiki provenance ("this feedback came from sessions where the <wiki> copy ran"). "Both" produces two separately-drafted diffs (the copy may have diverged — never one patch applied twice), each with its own gate, guard check, and commit — the base commits to the factory home's repo, the project copy commits with its project's repo.
  - **Spawn-time drift notice (backstop):** when `resolve-team` selects a project copy whose recorded `base-hash` no longer matches the current base, the resolution output carries a one-line notice ("base has changed since this copy forked — review the copy or acknowledge"); acknowledging updates the recorded hash ("deliberately forked"). This catches divergence created outside `/improve`; the routing flow above is the primary reconciliation mechanism.
- **Honest scope of the denylist:** `validate-persona`'s denylist covers registered-wiki basenames plus the optional `instructions/project-names.txt` — it is a best-effort tripwire, NOT the contamination gate for the public distribution. The repo leak-sweep (see the dev workflow) remains the actual gate; the spec must not oversell validation as isolation.
- `sync_agency_agents.py` (release-time, maintainer-only): pulls the agency-agents repo via the GitHub API and refreshes `plugin/assets/agency-agents/` (division directories preserved) plus an `ATTRIBUTION.md` (source repo, ref, license). The root repo is MIT; the sync script carries the license text and preserves any per-file authorship credits into `ATTRIBUTION.md`. Release checklist gains "re-sync the vendored catalog." `/team recruit`'s source list is amended to browse factory-home `references/` first (the user's curation wins) and the vendored catalog second — matching search-candidates' tie-break order, so recruit and staff agree.

## Testing

- Unit: search-candidates ranking + source filters; `--project` validation semantics (own name allowed, others still refuse; factory-home path unchanged); layered resolution precedence (project copy shadows base; base used when no copy; layer reported; a project copy mentioning its own project VALIDATES and RESOLVES end-to-end — the round-2 blocker regression test); list-copies routing lookup (finds copies across registered wikis by base-slug; empty when none); spawn-time drift notice semantics (hash mismatch notices, ack updates hash); jot wiki-provenance field (additive, omitted when absent); vendored-catalog guard (≥250 agent files present, each with parseable frontmatter incl. name+description; division = top-level directory — the files carry no division frontmatter; ATTRIBUTION.md present).
...
This feature ships as THREE plans: (1) machinery (team_ops/session-side CLI changes, vendor sync, layered resolution), (2) the `/staff` skill + `/team` and `/improve` SKILL amendments (layered spawning + layer-aware routing), (3) starter-roster authoring. The nine starter personas are content, not machinery — authoring them (genericized from the maintainer's private roster) is its own implementation plan with its own review: per-persona leak-sweep, `validate-persona` clean, and a blind cold-read ("does this archetype read as truly generic?"). The machinery plan treats `plugin/assets/starter-roster/` as an interface (any N validating personas), so the two plans land independently.
