...
- `validate_persona` gains an optional `--project <name>` argument: that single name is excluded from the denylist for this validation (used only for project-level copies). No frontmatter waivers exist; factory-home validation is unchanged and absolute.
- **Layered resolution** in `team_ops.py`: `resolve_team` / solo lookup / `assemble-context` check `<wiki-root>/personas/<slug>.md` first, then `<factory-home>/agents/<slug>.md` — full-file supersession by slug (no merging). Resolution output states which layer each member came from. The wiki lint's stray detection excludes the `personas/` dir (wiki-root scaffold addition, same class as `_templates/`).
- `sync_agency_agents.py` (release-time, maintainer-only): pulls the agency-agents repo via the GitHub API and refreshes `plugin/assets/agency-agents/` (division directories preserved) plus an `ATTRIBUTION.md` (source repo, ref, license). Release checklist gains "re-sync the vendored catalog."
