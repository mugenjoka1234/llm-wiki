...
- Unit: search-candidates ranking + source filters; `--project` validation semantics (own name allowed, others still refuse; factory-home path unchanged); layered resolution precedence (project copy shadows base; base used when no copy; layer reported); catalog guard (shipped catalog parses, ≥250 entries, required keys; `division` derived from each agent's top-level directory — the files themselves carry no division frontmatter).
