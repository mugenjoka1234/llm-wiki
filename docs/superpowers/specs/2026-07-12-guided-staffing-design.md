...
- **Project-copy provenance (informational only — automated drift detection cut):** a project copy's frontmatter records `base-slug:` and `forked:` date, so a reader always knows a copy's origin. Automated drift detection was considered and cut: it would need cross-wiki scan machinery `/improve` deliberately lacks, and a fork-time hash mislabels who moved after the first base edit. Copy upkeep is explicitly and only the user's job; `/improve` operates on the factory base, full stop.
